...
app.post("/api/projects", authMiddleware, (req, res) => {
  ...
  ).run(req.userId, name, JSON.stringify(shapes_data || []), background_color || "#ffffff");
...
app.get("/api/projects/:id", authMiddleware, (req, res) => {
  ...
  res.json({ project: { ...project, shapes_data: JSON.parse(project.shapes_data) } });
});
...
app.put("/api/projects/:id", authMiddleware, (req, res) => {
  ...
  ).run(name || null, shapes_data !== undefined ? JSON.stringify(shapes_data) : null, background_color || null, req.params.id);

  const updated = db.prepare("SELECT * FROM projects WHERE id = ?").get(req.params.id);
  res.json({ project: { ...updated, shapes_data: JSON.parse(updated.shapes_data) } });
});
